...
namespace project.Controllers
{
    public class CommentsController : Controller
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(text)) return BadRequest("Коментар пустий");

            int? userId = HttpContext.Session.GetInt32("UserId");
            if (userId == null) return StatusCode(401, "Користувач не авторизований або UserId не знайдено в сесії");
        ...
        public IActionResult Delete(int id)
        {
            ...
            if (comment.userID != userId.Value && !IsAdmin(userId.Value)) return Forbid();
